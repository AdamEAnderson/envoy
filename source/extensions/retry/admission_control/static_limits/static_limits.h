...
public:
  StaticLimits(uint64_t max_concurrent_retries)
    : max_active_retries_(max_concurrent_retries) {};
  ...
  Upstream::RetryStreamAdmissionControllerPtr createStreamAdmissionController(const StreamInfo::StreamInfo&) override {
    return std::make_unique<StreamAdmissionController>(
      active_retries_, max_active_retries_);
  };
...
private:
  ...
  public:
    StreamAdmissionController(PrimitiveGaugeSharedPtr active_retries, const uint64_t max_active_retries)
