...
public:
  ...
  void configureHostSet(const HostURLMetadataMap& host_metadata, MockHostSet& host_set) {
    ...
    if (hosts.empty()) {
      host_set.hosts_per_locality_ = makeHostsPerLocality({});
    } else {
  ...
  void zoneAwareInit(const std::vector<HostURLMetadataMap>& host_metadata_per_locality,
    ...
    for (uint32_t i = 0; i < host_metadata_per_locality.size(); ++i) {
      ...
      if (!locality_hosts.empty()) {
        ...
        no_local = no_local && i != 0;
      }
    }

    host_set_.hosts_ = hosts;
    host_set_.hosts_per_locality_ = makeHostsPerLocality(std::move(hosts_per_locality), no_local);
  ...
  void modifyHosts(HostVector add, HostVector remove, absl::optional<uint32_t> add_in_locality = {},
    ...
    bool originally_had_local = host_set.hosts_per_locality_->hasLocalLocality();
    for (const auto& host : remove) {
      ...
      bool noLocal = !originally_had_local;
      if (originally_had_local && locality_hosts_copy[0].empty()) {
        locality_hosts_copy.erase(locality_hosts_copy.begin());
        noLocal = true;
      }
      host_set.hosts_per_locality_ = makeHostsPerLocality(std::move(locality_hosts_copy), noLocal);
      host_set.healthy_hosts_per_locality_ = host_set.hosts_per_locality_;
    }
    ...
    bool no_local = !host_set.hosts_per_locality_->hasLocalLocality();
    for (const auto& host : add) {
      ...
      if (add_in_locality) {
        std::vector<HostVector> locality_hosts_copy = host_set.hosts_per_locality_->get();
        if (add_in_locality.value() == 0 && originally_had_local && no_local) {
          locality_hosts_copy.emplace(locality_hosts_copy.begin(), HostVector());
          no_local = false;
        }
        uint32_t corrected_add_in_locality = add_in_locality.value();
        if (originally_had_local && no_local) {
          corrected_add_in_locality--;
        }
        locality_hosts_copy[corrected_add_in_locality].emplace_back(host);
        host_set.hosts_per_locality_ = makeHostsPerLocality(std::move(locality_hosts_copy), no_local);
        host_set.healthy_hosts_per_locality_ = host_set.hosts_per_locality_;
      }
    }
  ...
  void modifyLocalHosts(HostVector add, HostVector remove, uint32_t add_in_locality) {
    bool originally_had_local = local_hosts_per_locality_->hasLocalLocality();
    for (const auto& host : remove) {
      ...
      bool no_local = !originally_had_local;
      if (locality_hosts_copy[0].empty()) {
        locality_hosts_copy.erase(locality_hosts_copy.begin());
        no_local = true;
      }
      local_hosts_per_locality_ = makeHostsPerLocality(std::move(locality_hosts_copy), no_local);
    }
    ...
    bool no_local = !local_hosts_per_locality_->hasLocalLocality();
    for (const auto& host : add) {
      local_hosts_->emplace_back(host);
      std::vector<HostVector> locality_hosts_copy = local_hosts_per_locality_->get();
      if (add_in_locality == 0 && originally_had_local && no_local) {
        locality_hosts_copy.emplace(locality_hosts_copy.begin(), HostVector());
        no_local = false;
      }
      uint32_t corrected_add_in_locality = add_in_locality;
      if (originally_had_local && no_local) {
        corrected_add_in_locality--;
      }
      locality_hosts_copy[corrected_add_in_locality].emplace_back(host);
